...
const minter = process.env.REACT_APP_ACCOUNT;
...
task("erc721", "Deploy ERC721 Token sample to chain")
    .setAction(async (taskArgs: any, { ethers }) => {
        const contractName = "MooToken";
        const erc721Factory = await ethers.getContractFactory(contractName);
        console.log(minter);
        const erc721 = await erc721Factory.deploy(minter, fsEndpoint);
        await erc721.deployTransaction.wait();
        console.log("ERC721 Token MooToken was deployed");
